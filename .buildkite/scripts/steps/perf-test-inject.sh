...
# --- core pinning --------------------------------------------------------------
# >=16 vCPU: envoy=0-1, prometheus floats, injectors pinned 2 cores each starting
# at core 2 (i1=2-3 i2=4-5 i3=6-7 i4=8-9 i5=10-11 i6=12-13), leaving 14-15 for
# kernel/docker/sampler. Each cpuset is overridable via the matching *_CPUS env.
...
if [ "$CORES" -ge 16 ]; then
  ...
  echo "--- core-pinning enabled (${CORES} vCPU): envoy=$ENVOY_CPUS injectors=2..13 (2 cores each)"
else
  # Not enough cores to pin 6 disjoint 2-core injectors — disable pinning and
  # cap the scale sweep so we don't oversubscribe a small agent.
...
# the inject dir.
echo "--- inject ceiling (N=1 RATE stair)"
"$RUN_INJECT" ceiling

# --- scale sweep ---------------------------------------------------------------
echo "--- inject scale sweep (N in: $SCALE_SET)"
...
if command -v buildkite-agent >/dev/null 2>&1; then
  ...
  echo "(local run) results at $REPO_ROOT/inject-ceiling.json, $REPO_ROOT/inject-scale.json"
fi
