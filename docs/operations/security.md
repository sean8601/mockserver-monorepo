...
> **`com.graphql-java:graphql-java` has no ceiling — it tracks latest (currently 26.0 in `pom.xml`).** graphql-java has required only Java 11 since version 21 and still ships Java 11 bytecode (class file version 55.0) in 25.x and 26.x, with an OSGi `Require-Capability` of `JavaSE version=11`, so every current major runs on the Java 17 floor. An earlier `>= 25.0.0` Dependabot ignore was a precautionary ceiling added before 25.x existed (on the assumption a future major *might* raise the floor); 25.0 and 26.0 shipped without doing so, so the ignore was removed and standalone bumps now auto-update. Re-introduce a ceiling only if a future graphql-java major actually raises its Java baseline above 17. Pulls in `com.graphql-java:java-dataloader`; ANTLR4 runtime and `reactive-streams` arrive transitively. Used by `org.mockserver.graphql.*` for schema-driven GraphQL response synthesis.
