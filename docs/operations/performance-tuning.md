...
- **Proxy / forward heavy → leave `forwardConnectionPoolEnabled` on (the default).** As of this change MockServer pools and reuses idle HTTP/1.1 keep-alive upstream connections by default. The k6 forward baseline showed that the previous per-request-connection behaviour exhausted ephemeral ports under sustained load (21% errors at 750 rps, 68% at 1500 rps, 212k `BindException`s from local port exhaustion); pooling drove this to ~0% errors with no latency regression in a controlled A/B. Only set `forwardConnectionPoolEnabled=false` if you specifically need a fresh upstream connection per request (the historical behaviour). Pooling only applies to plain HTTP/1.1 keep-alive upstreams — HTTP/2, HTTP/3, binary forwarding, streaming responses, proxy-tunnelled connections, and `Connection: close` upstreams always use a fresh connection.
