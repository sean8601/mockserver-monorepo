...
- **Proxy / forward heavy with well-behaved upstreams → consider enabling `forwardConnectionPoolEnabled=true`** (off by default). When enabled, MockServer pools and reuses idle HTTP/1.1 keep-alive upstream connections. The k6 forward baseline showed that the default per-request-connection behaviour can exhaust ephemeral ports under sustained load (21% errors at 750 rps, 68% at 1500 rps, 212k `BindException`s from local port exhaustion); pooling drove this to ~0% errors with no latency regression in a controlled A/B. It is **off by default** because it is only safe for upstreams that always return well-formed HTTP responses: MockServer's `error()` action (raw bytes / drop connection) — or any non-HTTP/malformed upstream reply — can poison a pooled keep-alive channel and desynchronise a later request that reuses it (lost or mismatched response). Enable it only when your forward targets always speak clean HTTP/1.1. Pooling only applies to plain HTTP/1.1 keep-alive upstreams — HTTP/2, HTTP/3, binary forwarding, streaming responses, proxy-tunnelled connections, `Connection: close` upstreams, and any reply that did not parse as a valid HTTP response always use a fresh connection.
