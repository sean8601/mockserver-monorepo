...
| `mockserver.loadGenerationSuppressEventLog` | `true` | When `true`, requests generated by an in-process load scenario are excluded from the driver's request event log, keeping the bounded log free for requests under test. Set to `false` to record load-generation traffic in the event log as well. The suppression marker is never serialised to the wire. |
