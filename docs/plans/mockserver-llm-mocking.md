...
| 7 | VCR mode + strict mode + body redaction + field normalisation | ✅ Shipped — U4 cassette manager + strict mode (`mockserver.llmVcrStrict` / `load` `strict`, 599 catch-all), body-field redaction (`FixtureRedactor` + `mockserver.fixtureBodyRedactFields` / `record` `redactBodyFields`), replay normalisation (`load` `normalizeRequestBodyFields`) |

### Tier 3 — valuable / specialised

| # | Item | Status |
|---|---|---|
| 8 | MCP/A2A conformance contract testing (`run_mcp_contract_test`) | ✅ Shipped (MCP) — `run_mcp_contract_test` MCP tool over a network-free `McpContractTest` orchestrator: runs the JSON-RPC handshake + core methods (initialize, notifications/initialized, ping, tools/list, unknown-method `-32601`) against a target MCP Streamable-HTTP endpoint and validates response shape, never tool semantics; optional `tools/call` shape check. Deterministic. A2A conformance not pursued (different spec; revisit if demand) |
...
| 12 | Prompt-injection / adversarial-response harness | ✅ Shipped — `AdversarialResponseLibrary` (curated benign payloads: injection / jailbreak / exfiltration / malformed / oversized) + `mock_adversarial_llm_response` MCP tool. Defensive testing aid; deterministic |
