...
**Decision (2026-05-28): Option A+ — keep plugin, publish XML data, enforce thresholds.** M1 added artifact upload globs for `**/target/site/jacoco/jacoco.xml` and `**/target/site/jacoco-it/jacoco.xml` to `.buildkite/pipeline-java.yml`. (The initial wider `jacoco/**` and `jacoco-it/**` globs hit Buildkite's 5000-artifact-per-job limit because JaCoCo emits one HTML page per source class — narrowed to the machine-readable XML data files only.) M1 also wired `jacoco:check` with safe-low thresholds in `mockserver-core/pom.xml` and `mockserver-netty/pom.xml`. M6 ratcheted those thresholds to ~3-4 points below the measured unit-test floors (mockserver-core LINE ≥0.65, mockserver-netty LINE ≥0.55). The ~5-10% build cost is now paid back by a real coverage gate and per-build XML coverage data for downstream tooling.
