...
- Added a **`decompressRequestBodies`** configuration property (default `true`, preserving existing behaviour). When set to `false`, MockServer no longer decompresses incoming request bodies (HTTP/1.1 and HTTP/2): the body is matched and recorded exactly as received on the wire (still compressed), the `Content-Encoding` header is preserved, and `HttpRequest#getBodyAsRawBytes()` returns the original compressed bytes — so you can verify that a client actually sent a compressed body. With it disabled, body matchers operate on the compressed bytes rather than the decompressed content. See the configuration properties page (fixes #2326).
