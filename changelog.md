...
- **BREAKING: Matcher-driven breakpoints replace the global breakpoint flags** — breakpoints are no longer toggled by the four global config flags (`breakpointEnabled`, `breakpointResponseEnabled`, `breakpointStreamEnabled`, `breakpointInboundEnabled`), which have been **removed**. Instead you register a **request matcher** (works exactly like an expectation request matcher) together with the phases to break at: `PUT /mockserver/breakpoint/matcher` with `{httpRequest, phases:["REQUEST"|"RESPONSE"|"RESPONSE_STREAM"|"INBOUND_STREAM"], clientId:"..."}`. A forwarded/proxied exchange whose request matches a registered breakpoint pauses at the selected phase(s). Manage matchers via `GET`/`PUT /mockserver/breakpoint/matchers`, `PUT /mockserver/breakpoint/matcher/remove` (`{id}`), and `PUT /mockserver/breakpoint/matcher/clear`; the registry is cleared on `/mockserver/reset`. The `breakpointTimeoutMillis` (30000) and `breakpointMaxHeld` (50) safety rails are retained.
- **BREAKING: `clientId` is now required for breakpoint registration; REST resolution endpoints removed** — `PUT /mockserver/breakpoint/matcher` now requires a `clientId` field (the callback WebSocket client id); omitting it returns 400. The callback WebSocket is the only resolution transport. The following REST endpoints have been removed: `GET/PUT /mockserver/breakpoint` (list paused), `PUT /mockserver/breakpoint/continue`, `PUT /mockserver/breakpoint/modify`, `PUT /mockserver/breakpoint/abort`, `GET /mockserver/breakpoint/streams` (list held frames), and `PUT /mockserver/breakpoint/stream/{continue,modify,drop,inject,close}`. The corresponding Java client methods (`listBreakpoints`, `continueBreakpoint`, `modifyBreakpoint`, `modifyBreakpointResponse`, `abortBreakpoint`, `listStreamFrameBreakpoints`, `continueStreamFrame`, `modifyStreamFrame`, `dropStreamFrame`, `injectStreamFrame`, `closeStreamFrame`) have been removed. All clients (including the dashboard) now resolve breakpoints over the callback WebSocket.
- **Interactive breakpoint resolution over the callback WebSocket** — a breakpoint matcher can carry an optional owner `clientId`. When set, a matching forwarded REQUEST or RESPONSE exchange is dispatched to that connected callback-WebSocket client (the same `/_mockserver_callback_websocket` channel `forwardObject`/`responseObject` clients use) for interactive resolution: the client replies with a modified request (forward), a response (abort/replace), or the original (continue). When no `clientId` is set, the existing REST poll/continue/modify/abort flow is used unchanged. Shares the `breakpointTimeoutMillis` auto-continue and `breakpointMaxHeld` cap rails; a client disconnect removes its breakpoints and auto-continues anything it was holding. This is the foundation for both language clients and the dashboard acting as callback clients over one shared core dispatch path.
- **Per-frame streaming breakpoints over the callback WebSocket** — RESPONSE_STREAM (outbound) and INBOUND_STREAM (client→server) breakpoints also resolve interactively over the callback WebSocket when the matcher carries a `clientId`, across all nine streaming hold points (SSE/chunked, HTTP/3 gRPC, gRPC server-streaming, WebSocket eager/bidi, GraphQL-subscription, and the WebSocket/GraphQL/gRPC-bidi inbound paths). Two new WS message types form the frozen per-frame protocol: a server→client `PausedStreamFrameDTO` (`correlationId`, `streamId`, `sequenceNumber`, `direction`, `phase`, base64 `body`, request method/path) and a client→server `StreamFrameDecisionDTO` (`correlationId`, `action` ∈ CONTINUE/MODIFY/DROP/INJECT/CLOSE, optional base64 `body`). Event-loop safe (decisions marshalled onto the channel event loop, frame bytes copied to `byte[]`), with ordering and backpressure preserved and the shared timeout/max-held rails + client-disconnect auto-continue. The per-server WebSocket registry is injected per-channel (no process-global state). When the matcher has no `clientId`, the existing REST stream-frame park/resolve path is used unchanged.
- **Java client breakpoint API (matcher + callback handlers)** — `MockServerClient.addBreakpoint(matcher, phases…, handlers…)` registers a breakpoint matcher and resolves paused exchanges interactively over the callback WebSocket, with typed handlers per phase: `BreakpointRequestHandler` (return a request to forward/modify or a response to abort), `BreakpointResponseHandler` (return the response to write), and `BreakpointStreamFrameHandler` (return a CONTINUE/MODIFY/DROP/INJECT/CLOSE decision). Plus `listBreakpointMatchers()`, `removeBreakpointMatcher(id)`, `clearBreakpointMatchers()`. The client lazily opens one callback-WS connection (reused across breakpoints) and tears it down on stop/reset. **Per-matcher handler routing:** each pushed paused item carries the matched breakpoint's id (a new `X-MockServer-BreakpointId` header for request/response and a `breakpointId` field on the stream-frame message), so each breakpoint routes to its own handler rather than a single shared per-phase handler — breakpoints behave as independent matchers. This is the reference API the other language clients mirror.
...
- **Go, .NET & Rust client breakpoint APIs (new callback-WebSocket stacks)** — the Go, .NET, and Rust clients — previously REST-only — gain a full callback-WebSocket stack (Go `gorilla/websocket`, .NET built-in `ClientWebSocket`, Rust `tungstenite`) plus the matcher-driven breakpoint API (`addBreakpoint`/`AddBreakpoint`/`add_breakpoint` + convenience overloads, list/remove/clear breakpoint matchers). Each connects to `/_mockserver_callback_websocket`, registers a `clientId`, and resolves paused request/response/stream-frame exchanges with per-matcher handler routing (by `X-MockServer-BreakpointId` header / `breakpointId` frame field), auto-continuing on handler error/panic. Concurrency-safe (serialised WS writes + lazy init; Go verified with `-race`) and reconnect-on-dead-connection. PHP is excluded (no WebSocket support). This completes breakpoint support across all eight non-PHP clients (Java, Node, Python, Ruby, Go, .NET, Rust).
- **Forwarded-stream frame breakpoints (backend)** — per-frame hold/modify/drop/inject/close for forwarded streaming responses (SSE and HTTP/1.1 chunked). Each frame is intercepted in the streaming relay path, parked in a `StreamFrameBreakpointRegistry`, and resolved via new control-plane REST endpoints (`/mockserver/breakpoint/stream/{continue,modify,drop,inject,close}` and `GET /mockserver/breakpoint/streams`). Fully non-blocking (event-loop safe), with backpressure (autoRead paused while a frame is held), ordered frame resolution, stream-close eviction, timeout auto-continue, and DoS cap. Activated when a matching `RESPONSE_STREAM` breakpoint matcher is registered (zero overhead otherwise). Scoped to HTTP/1.1; gRPC/HTTP-2/WebSocket are explicit follow-ups (A1d/A1e).
- **Dashboard Breakpoints panel (callback-WebSocket client)** — the dashboard is now a real callback client: it connects to `/_mockserver_callback_websocket` (the server assigns it a `clientId`, since a browser WebSocket can't send the registration header) and resolves paused exchanges live over the same callback path the language clients use — no REST polling. The panel has three tabs: **Matchers** (register a breakpoint matcher with a method/path matcher + phase checkboxes; list/remove/clear), **Live Exchanges** (paused requests/responses arrive in real time — Continue / Modify the JSON / Abort), and **Live Streams** (paused stream frames — Continue / Modify / Drop / Inject / Close). A connection-state indicator shows the callback-WS status.
- **Response-phase breakpoint support in dashboard and Java client** — the Breakpoints panel now displays each exchange's phase (REQUEST or RESPONSE); for response-phase exchanges the held response (status code, reason phrase) is shown and the Modify action edits the response JSON (`PUT /mockserver/breakpoint/modify` with `{id, httpResponse}`). The Java client gains `modifyBreakpointResponse(id, HttpResponse)` to complement the existing request-phase `modifyBreakpoint(id, HttpRequest)`.
...
- **Java client stream-frame breakpoint methods** — `MockServerClient` now exposes typed methods for the forwarded-stream frame breakpoint endpoints: `listStreamFrameBreakpoints()`, `continueStreamFrame(id)`, `modifyStreamFrame(id, body)`, `dropStreamFrame(id)`, `injectStreamFrame(id, body)`, and `closeStreamFrame(id)`. These wrap the existing REST endpoints (`GET /mockserver/breakpoint/streams`, `PUT /mockserver/breakpoint/stream/{continue,modify,drop,inject,close}`) using the same Jackson `ObjectNode` plumbing as the request breakpoint methods.
- **Inbound bidirectional frame breakpoints (backend)** — intercepts client-to-server frames on WebSocket, GraphQL-subscription, and gRPC-bidi connections before MockServer processes them (the final breakpoint increment, completing A1a-A1e). Each inbound frame is copied to byte[], the original ByteBuf/Http2DataFrame is released immediately (refunding the HTTP/2 flow-control window), and the copy is parked in `StreamFrameBreakpointRegistry` with `direction=INBOUND`. Supports continue/modify/drop/inject/close via the existing stream frame REST endpoints. Fully non-blocking with backpressure (autoRead paused for WebSocket/GraphQL; pull-based ctx.read() withholding for gRPC-bidi), channel-close eviction, and direction field in the list endpoint. Activated when a matching `INBOUND_STREAM` breakpoint matcher is registered (zero overhead otherwise). The gRPC-bidi implementation is HTTP/2 flow-control safe: the DATA frame is released immediately (window refunded), and backpressure is orthogonal via the existing pull-based model.
- **Dashboard direction badge on stream frames** — the Breakpoints panel Streams tab now shows an Inbound/Outbound direction chip on each held frame, so users can distinguish client-to-server frames from server-to-client frames at a glance. Inbound frames display a blue "Inbound" badge; outbound frames show a neutral "Outbound" badge. Frames without a direction field gracefully default to Outbound.
- **HTTP/3 gRPC server-streaming breakpoints** — extends the stream-frame breakpoint engine to the HTTP/3 (QUIC) gRPC server-streaming path. When a matching `RESPONSE_STREAM` breakpoint matcher is registered, each outbound DATA frame is parked in `StreamFrameBreakpointRegistry` (stream-id suffix `-h3-grpc-stream`) before writing. Supports per-frame continue/modify/drop/inject/close via the existing REST endpoints. Frame bytes are `byte[]` (no ByteBuf retained). Decision callbacks run on the QUIC stream's event loop. Held frames evicted on stream close. Default-off, zero overhead when disabled.
...
- **Internal docs corrections** — corrected `docs/code/breakpoints.md`: removed stale "Future work" section (all four items shipped — HTTP/3-gRPC, gRPC-bidi inbound, and both dashboard UI features); added `GrpcBidiStreamHandler.handleData` and `GrpcBidiRouterHandler` to the Inbound frame breakpoints key-classes; corrected `docs/code/configuration-reference.md` Breakpoints row to include the missing `breakpointInboundEnabled` property; updated `docs/README.md` doc counts (code: 21→24, operations: 13→15); replaced "error-class" with "destructive" in `docs/code/metrics.md` to match `ChaosAutoHaltMonitor.DESTRUCTIVE_FAULT_TYPES`; updated `docs/code/dashboard-ui.md` Streams tab description to reflect the shipped direction badge and gRPC-bidi inbound frames; added three missing code-doc rows (`ai-protocol-mocking.md`, `llm-codec-fixtures.md`, `llm-security-audit.md`) to the `AGENTS.md` reference table.
