...
- **Inbound bidirectional frame breakpoints (backend)** — intercepts client-to-server frames on WebSocket and GraphQL-subscription connections before MockServer processes them (the final breakpoint increment, completing A1a-A1e). Each inbound frame is copied to byte[], the original ByteBuf is released, and the copy is parked in `StreamFrameBreakpointRegistry` with `direction=INBOUND`. Supports continue/modify/drop/inject/close via the existing stream frame REST endpoints. Fully non-blocking with backpressure (autoRead paused while a frame is held), channel-close eviction, and direction field in the list endpoint. Gated by `breakpointInboundEnabled` (default false, zero overhead). gRPC-bidi inbound is deferred due to HTTP/2 flow-control complexity.
