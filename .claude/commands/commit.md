...
1. **Classify** — `git status --short`; identify only files YOU changed this session; classify by category (java/terraform/bash/docker/docs/config/helm/npm/python/ruby/**control**). Files under `.opencode/rules/**`, `.opencode/agents/**`, `.claude/agents/**`, `opencode.jsonc`, the review constitution, or CI/test gates are the **control** (AI-component) class — higher-scrutiny (see steps 2 and 4).
2. **Validate** — run category-specific, executable validations:
   ...
   - **control**: run the evaluation harness for control / AI-component changes — `STRICT=1 bash .opencode/evals/run-evals.sh` (a regression blocks)
