...
8. **Integration adversarial review, under the held lock (§14.6).** Spawn a *separate, freshly-spawned, clean-context* reviewer — the **`review-final`** agent **applying the `review-integration` constitution** (the constitution is a profile, not a separate agent; do not use `review-final`'s default profile, and do not reuse the implementer or any context-anchored continuation of it) on the integrated diff (`git diff origin/master...HEAD` after the rebase), giving it the step-7 re-verify results and the plan/wave scope. It probes the Tier-2/3 concerns: combination defects, interface drift, duplicated/conflicting changes, dropped units, end-to-end coherence vs the plan. Loop to convergence or the 8-iteration cap (§14.5). **On BLOCK:** do not push; release the lock; leave the worktree for fix-forward.
